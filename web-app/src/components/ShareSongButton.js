...
export default function ShareSongButton({ song }) {
    ...
    const handleShareClick = async () => {
      ...
      const shareAudio = async () => {
        try {
          ...
          if (!('navigator' in window && 'share' in navigator && 'canShare' in navigator)) {
    ...
    return (
      <button
        type="button"
        className="share-song-button"
        onClick={handleShareClick}
        disabled={isResolving}
        aria-label="Distribuie melodia"
      >
        {isResolving ? (
          <div className="share-loading">Se încarcă...</div>
        ) : (
          <svg 
